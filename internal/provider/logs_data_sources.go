...
// ---- skycloak_cluster_build ----

var (
	_ datasource.DataSource              = (*clusterBuildDataSource)(nil)
	_ datasource.DataSourceWithConfigure = (*clusterBuildDataSource)(nil)
)

type clusterBuildDataSource struct{ client *skycloak.Client }

// NewClusterBuildDataSource returns the skycloak_cluster_build data source.
func NewClusterBuildDataSource() datasource.DataSource { return &clusterBuildDataSource{} }

type clusterBuildModel struct {
	ClusterID   types.String `tfsdk:"cluster_id"`
...
func (d *clusterBuildDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_cluster_build"
}

func (d *clusterBuildDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "A single cluster image build, including its log lines.",
		Attributes: map[string]schema.Attribute{
			"cluster_id":   schema.StringAttribute{Required: true, MarkdownDescription: "Cluster ID."},
			"build_id":     schema.StringAttribute{Required: true, MarkdownDescription: "Build ID."},
			"status":       schema.StringAttribute{Computed: true, MarkdownDescription: "Build status."},
			"phase":        schema.StringAttribute{Computed: true, MarkdownDescription: "Current phase."},
			...
			"logs":         schema.ListAttribute{Computed: true, ElementType: types.StringType, MarkdownDescription: "Build log lines."},
		},
	}
}

func (d *clusterBuildDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	d.client = configuredClient(req, &resp.Diagnostics)
}

func (d *clusterBuildDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var cfg clusterBuildModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &cfg)...)
	if resp.Diagnostics.HasError() {
		return
	}
	b, logs, err := d.client.GetClusterBuild(ctx, cfg.ClusterID.ValueString(), cfg.BuildID.ValueString())
	if err != nil {
		resp.Diagnostics.AddError("Unable to read cluster build", err.Error())
		return
	}
	cfg.Status = types.StringValue(b.Status)
	cfg.Phase = types.StringValue(b.Phase)
	cfg.Progress = types.Int64Value(b.Progress)
	cfg.Error = optionalString(b.Error)
	cfg.StartedAt = types.StringValue(b.StartedAt)
	cfg.CompletedAt = optionalString(b.CompletedAt)
	cfg.Logs = sliceToStringList(ctx, logs, &resp.Diagnostics)
